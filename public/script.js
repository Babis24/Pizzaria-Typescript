document.addEventListener('DOMContentLoaded', () => {
    ...
    // -- Lógica de Checkout e Comprovante --
    async function handleCheckout() {
        ...
        try {
            ...
            if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.erro || 'Falha ao finalizar o pedido.'); }
    ...
    function showReceipt(pedidoResult, cliente) {
        receiptContent.innerHTML = `
            ...
            <p><strong>Endereço de Entrega:</strong> ${cliente.endereco || 'Não informado no cadastro'}</p>
            <hr><h4>Itens:</h4>
            <ul style="list-style: none; padding: 0;">
                ${pedidoResult.itens.map(item => {
                    const productDetails = cart.find(cartItem => cartItem.id === item.produtoId);
                    return `<li style="display:flex; justify-content: space-between; padding: 4px 0;"><span>${productDetails.name} (x${item.quantidade})</span></li>`;
                }).join('')}
            </ul><hr>
